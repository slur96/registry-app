pipeline {
	...
	stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-54-162-222-86.compute-1.amazonaws.com:8080/job/gitops-registry-app-cd/buildWithParameters?token=gitops-token'"
                }
            }
       }
    }
